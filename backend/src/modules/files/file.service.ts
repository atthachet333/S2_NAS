      ...
      const created = await prisma.$transaction(async (tx) => {
        const resource = await tx.resource.create({
          data: {
            ...
            /*
             * สืบทอดเพดานการเปิดเผยจากโฟลเดอร์แม่ด้วย ด้วยเหตุผลเดียวกับการมองเห็น (F25-D)
             *
             * ไฟล์ที่อัปโหลดเข้าโฟลเดอร์ที่ตั้งใจเปิดสาธารณะควรเปิดได้ตามเจตนานั้น
             * และไฟล์ที่เข้าโฟลเดอร์ชั้นลับต้องไม่หลุดเป็นสาธารณะเพราะค่าเริ่มต้นของระบบ
             * ที่ระดับรากไม่มีแม่ให้สืบทอด จึงใช้ค่าอนุรักษ์นิยมคือเปิดออกนอกองค์กรไม่ได้
             */
            classification: parent?.classification ?? 'INTERNAL',

 ...
 *
 * **เปลี่ยนใน Link Lock:** เส้นทางกลุ่มนี้ไม่เปิดให้ผู้ไม่ระบุตัวตนอีกแล้ว
 * ทุกเส้นต้องเข้าสู่ระบบก่อน แล้วจึงถูกตัดสินด้วยสิทธิ์ของผู้ใช้เอง
 */
...
describe('F11 การทำให้พื้นที่ลูกค้าแข็งแรงขึ้น', () => {
  ...
  describe('บัญชีลูกค้าถูกปฏิเสธจากทุกเส้นทางภายในที่ลงทะเบียนจริง', () => {
    ...
    /**
     * ลิงก์แชร์ภายนอกไม่ใช่ประตูหลังของพื้นที่ลูกค้า (F18 §80 → ปรับใน Link Lock)
     *
     * **ข้อสันนิษฐานเดิมกลับด้านแล้ว โดยตั้งใจ**
     *
     * เดิมข้อนี้ยืนยันว่า "ลูกค้าที่ล็อกอินอยู่ต้องได้คำตอบเหมือนแขกทุกประการ" ซึ่งสมเหตุสมผล
     * ในโลกที่ลิงก์เป็นกุญแจ - ตัวตนไม่ควรเปลี่ยนอะไร
     *
     * ตั้งแต่ Link Lock ลิงก์ไม่ใช่กุญแจอีกต่อไป ตัวตน *คือ* สิ่งที่ตัดสิน ผู้ไม่ระบุตัวตน
     * ได้ 401 เสมอ ส่วนผู้ที่ล็อกอินแล้วถูกตัดสินด้วยสิทธิ์ของตัวเอง สองคำตอบนี้จึงต้องต่างกัน
     *
     * สิ่งที่ยังต้องจริงเหมือนเดิมคือใจความ: **ลิงก์ไม่เพิ่มสิทธิ์ให้บัญชีลูกค้า**
     * ข้อนี้จึงยืนยันใจความนั้นแทน - ลูกค้าที่ล็อกอินอยู่ต้องไม่ได้เนื้อหาจากลิงก์ที่ไม่ใช่ของตน
     */
    test('บัญชีลูกค้าที่ล็อกอินอยู่ไม่ได้สิทธิ์เพิ่มจากเส้นทางลิงก์แชร์', async () => {
      ...
      // โทเคนที่ยาวพอผ่าน schema แต่ไม่มีอยู่จริง
      ...
      for (const route of routes) {
        ...
        // ผู้ไม่ระบุตัวตนต้องถูกหยุดที่ด่านเข้าสู่ระบบเสมอ ก่อนแตะฐานข้อมูลด้วยซ้ำ
        assert.equal(
          asGuest.statusCode, 401,
          `${route.method} ${route.path}: แขกต้องได้ 401 แต่ได้ ${asGuest.statusCode}`,
        );
        assert.equal(asGuest.json().error?.code, 'LOGIN_REQUIRED');

        // ลูกค้าที่ล็อกอินแล้วต้องไม่ได้เนื้อหาของลิงก์ที่ไม่ใช่ของตน
        assert.ok(
          asClient.statusCode >= 400,
          `${route.method} ${route.path}: ลูกค้าไม่ควรได้ ${asClient.statusCode}`,
        );
        assert.equal(
          asClient.body.includes('เนื้อหา') || asClient.body.includes('storageKey'),
          false,
          `${route.method} ${route.path}: ลูกค้าต้องไม่ได้เนื้อหาหรือข้อมูลภายในของที่เก็บ`,
        );
      }
    });

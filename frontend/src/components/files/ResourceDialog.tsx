...
export function ResourceDialog({ mode, entry, parentId, driveRoot = 'MY_DRIVE', onClose, onSuccess }: { mode: ResourceDialogMode; entry: DriveEntry | null; parentId: string | null; driveRoot?: DriveRoot; onClose: () => void; onSuccess: (message: string) => void }) {
  ...
  return <div className="fixed inset-0 z-[var(--z-dialog)] flex items-end justify-center bg-[var(--s2-overlay)] p-0 backdrop-blur-sm sm:items-center sm:p-3" onMouseDown={onClose}>
    <section role="dialog" aria-modal="true" aria-labelledby="resource-dialog-title" className="max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-line sm:rounded-2xl bg-[var(--s2-elevated)] p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-pop sm:p-6 sm:pb-6" onMouseDown={(event) => event.stopPropagation()}>

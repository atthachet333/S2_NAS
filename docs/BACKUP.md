...
ข้อความจาก OCR อยู่ในตารางเดียวกับดัชนีข้อความปกติ จึงติดไปกับชุดสำรองโดยไม่ต้องทำอะไรเพิ่ม
และเป็น **ข้อมูลที่สร้างใหม่ได้** เช่นเดียวกัน - การกู้คืนต้องไม่ขึ้นกับมัน

**ไฟล์ภาพชั่วคราวระหว่างทำ OCR ไม่ถูกสำรอง** มันถูกลบทันทีที่อ่านเสร็จ
และโฟลเดอร์ที่ค้างจากการล่มกลางคันถูกเก็บกวาดตอนเริ่มระบบ

---

## F14 - การตรวจแก้ข้อความต้องอยู่ในชุดสำรองเสมอ

หัวข้อ F12 ด้านบนอธิบายว่าดัชนีข้อความเป็นข้อมูลที่สร้างใหม่ได้ ซึ่งยังจริงสำหรับ
ข้อความที่เครื่องสกัดเอง

**ข้อความที่คนตรวจแก้แล้วไม่ใช่ข้อมูลแบบนั้น** มันสร้างใหม่โดยอัตโนมัติไม่ได้เลย

ตารางที่ต้องอยู่ในชุดสำรอง:

| ตาราง | เก็บอะไร |
|---|---|
| `resource_search_index` | ข้อความที่มีผลใช้งาน ผลดิบ และเลขรุ่นการตรวจแก้ |
| `resource_text_corrections` | ประวัติทุกรุ่นที่เคยบันทึก |

ทั้งสองตารางอยู่ใน dump ของฐานข้อมูลทั้งชุดตามปกติ ไม่มีการยกเว้นใด ๆ

การซ้อมกู้คืนตรวจว่าข้อความกลับมาครบทุกตัวอักษร ไม่ใช่แค่ "มีตารางอยู่"
ดู [RESTORE.md](RESTORE.md) และชุดทดสอบ `src/modules/backup/correction-restore.test.ts`

> **ห้ามยกเว้นสองตารางนี้ออกจากชุดสำรองเพื่อลดขนาดไฟล์**
> ขนาดที่ประหยัดได้เล็กมาก ส่วนสิ่งที่เสียคือแรงงานของคนที่กู้กลับมาไม่ได้

---

## F15 - ข้อมูลที่คนสร้างในเฟสนี้

สามอย่างต่อไปนี้เป็นแรงงานของคน ไม่ใช่ข้อมูลที่เครื่องคำนวณใหม่ได้
จึงต้องอยู่ในชุดสำรองเสมอ:

| ตาราง / ฟิลด์ | เก็บอะไร |
|---|---|
| `saved_searches` | ชุดค้นหาที่คนคิดเงื่อนไขและตั้งชื่อเอง |
| `document_categories` | ประเภทเอกสารที่องค์กรกำหนด |
| `resources.documentCategoryId` | การจัดประเภทที่คนตัดสินใจทีละฉบับ |
| `resource_search_index.reviewStatus` | สถานะการตรวจ พร้อมผู้ตรวจและเวลา |

ทั้งหมดอยู่ใน dump ของฐานข้อมูลทั้งชุดตามปกติ ไม่มีการยกเว้นใด ๆ

> **ห้ามยกเว้นตารางเหล่านี้ออกจากชุดสำรองเพื่อลดขนาดไฟล์**
> ขนาดที่ประหยัดได้เล็กมาก ส่วนสิ่งที่เสียคือเวลาที่คนนั่งอ่านเอกสารทีละฉบับ

การซ้อมกู้คืนตรวจทั้งสี่อย่างนี้จริง ดู `src/modules/backup/f15-restore.test.ts`

---

## F16 - สถานะการกำกับดูแลเป็นข้อมูลธุรกิจหลัก

นโยบายการเก็บรักษา สถานะคลัง และการระงับการลบ **ไม่ใช่ข้อมูลที่คำนวณใหม่ได้**

| ตาราง / ฟิลด์ | เก็บอะไร |
|---|---|
| `retention_policies` | นิยามนโยบายที่องค์กรกำหนด |
| `legal_holds` | ประวัติการระงับทั้งหมด รวมที่ปลดไปแล้ว |
| `resources.lifecycleState` · `archivedAt` | สถานะคลัง |
| `resources.retentionUntil` · `retentionForever` | วันหมดอายุที่คำนวณไว้ |

ถ้าสิ่งเหล่านี้หายไปตอนกู้คืน ผลไม่ใช่แค่ความไม่สะดวก: เอกสารที่ถูกระงับการลบ
ตามคำสั่งทางกฎหมายจะกลายเป็นเอกสารธรรมดาที่ลบได้ และไม่มีใครรู้ว่าเคยมีการระงับอยู่

> **อย่าสับสนกับอายุของชุดสำรอง** ซึ่งเป็นคนละเรื่องและตั้งค่าคนละที่
> ดู [DOCUMENT_RETENTION.md](DOCUMENT_RETENTION.md)

การซ้อมกู้คืนตรวจทั้งหมดนี้จริง ดู `src/modules/backup/f16-restore.test.ts`

## บันทึกการตรวจสอบต้องรอด (F17)
...
และไม่มีทางสร้างกลับมา เพราะเหตุการณ์ที่ผ่านไปแล้วเกิดขึ้นครั้งเดียว

ชุดสำรองครอบคลุมทั้งตารางอยู่แล้ว รวมถึง `ipAddress`, `userAgent` และ `metadata` ภาษาไทย

**ไฟล์ CSV ที่ส่งออกไม่อยู่ในชุดสำรอง** และไม่ถูกเก็บเป็นทรัพยากรของ NAS -
มันเป็นสำเนาที่สร้างใหม่ได้เสมอ การเก็บไว้มีแต่จะสร้างสำเนาข้อมูลอ่อนไหวที่ไม่มีใครดูแล
และไม่มีใครจำได้ว่ามีอยู่ ดู [AUDIT_EXPORT.md](AUDIT_EXPORT.md)

การซ้อมตรวจเรื่องนี้จริง ดู `src/modules/backup/f17-restore.test.ts`

## ลิงก์แชร์ภายนอกต้องรอด (F18)

`public_share_links` เป็นข้อมูลหลัก ชุดสำรองครอบคลุมทั้งตาราง:

| ฟิลด์ | ทำไมต้องรอด |
|---|---|
| `tokenHash` | ลิงก์ที่ส่งให้ลูกค้าไปแล้วยังใช้ได้หลังกู้คืน |
| `passwordHash` | ลิงก์ที่มีรหัสผ่านยังต้องการรหัสเดิม |
| `allowPreview` `allowDownload` | สิทธิ์ที่ผู้สร้างตั้งใจกำหนด |
| `maxViews` `maxDownloads` | เพดานที่ตกลงไว้ |
| `viewCount` `downloadCount` | โควตาที่ใช้ไปแล้วต้องไม่ถูกคืนให้ฟรี |
| `revokedAt` `revokedById` | ลิงก์ที่ปิดไปแล้วต้องไม่ฟื้นคืนชีพ |

**โทเคนดิบไม่มีอยู่ในฐานข้อมูล** ชุดสำรองจึงไม่ได้พกกุญแจที่ใช้งานได้ติดไปด้วย
ชุดสำรองที่รั่วไม่กลายเป็นพวงกุญแจเปิดเอกสาร

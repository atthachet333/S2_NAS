...
The database stores logical metadata; the storage provider stores opaque version bytes. This document describes the local provider, whose bytes live below `S2_NAS_STORAGE_ROOT`; see [OBJECT_STORAGE.md](OBJECT_STORAGE.md) for the provider model and the S3-compatible provider. The user-visible folder tree never maps to physical directories. Rename, move, restore, and ownership transfer therefore cause no physical relocation.
...
Content and downloads use authenticated API streams. Storage is never served statically. `resolveStorageKey` and `resolveInsideStorage` contain all reads/writes within the configured root, and are reachable only from the local provider. DTOs and system status responses expose no storage key or physical path.

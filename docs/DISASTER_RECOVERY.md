...
## F16 - การกำกับดูแลอยู่ในเส้นทางวิกฤต

ต่างจาก OCR และดัชนีข้อความ - **นโยบายการเก็บรักษาและ Legal Hold ต้องกลับมาพร้อมระบบ**

ห้ามเปิดระบบให้ผู้ใช้ทั่วไปเข้าใช้ก่อนตรวจว่าสถานะเหล่านี้กู้กลับมาครบ
เพราะระบบที่เปิดโดยไม่มีการคุ้มครอง คือระบบที่อนุญาตให้ลบเอกสารที่ห้ามลบ

ลำดับที่ปลอดภัย:

1. กู้ฐานข้อมูลและไฟล์
2. ตรวจสี่ข้อใน [RESTORE.md](RESTORE.md) หัวข้อ F16
3. **ยืนยันว่างานเก็บกวาดถังขยะยังไม่ทำงาน** จนกว่าจะตรวจเสร็จ
   (ตั้ง `TRASH_RETENTION_DAYS = 0` ชั่วคราวได้)
4. เปิดให้ใช้งาน แล้วจึงเปิดงานเก็บกวาดกลับ

งานเก็บกวาดข้ามเอกสารที่ถูกคุ้มครองอยู่แล้วตามปกติ ขั้นตอนที่ 3 เป็นการป้องกัน
กรณีที่การกู้คืนไม่สมบูรณ์และการคุ้มครองบางส่วนหายไปโดยไม่มีใครทันสังเกต
# F21 recovery order

กู้ database/storage ก่อน ตรวจ thread/citation แล้ว provision โมเดลผู้ช่วยใหม่เป็นขั้นตอนท้าย ความล้มเหลวของโมเดลไม่ใช่เหตุให้ core NAS unavailable
